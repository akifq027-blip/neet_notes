  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        // Strip out demo notes (22, 23) while keeping user's own uploaded notes
        const cleaned = parsed.filter((n: Note) => n.id !== 22 && n.id !== 23);
        FALLBACK_NOTES = cleaned.length > 0 ? cleaned : FALLBACK_NOTES;
