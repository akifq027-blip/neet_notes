...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      if (libRes.success) setLibraryNotes(libRes.library);
      if (ordRes.success) setOrders(ordRes.orders);

      // Wishlist notes (mock/client filtered or fetched)
      if (allNotesRes.success) {
        setWishlistNotes(allNotesRes.notes.slice(0, 3));
      }
    } catch (err) {
  ...
  const filteredLibrary = libraryNotes.filter((n) =>
    n.title.toLowerCase().includes(searchLibrary.toLowerCase()) ||
    n.chapter.toLowerCase().includes(searchLibrary.toLowerCase()) ||
    n.subject.toLowerCase().includes(searchLibrary.toLowerCase())
  );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="bg-emerald-500/20 text-emerald-300 text-xs font-bold px-2 py-0.5 rounded">
                NEET Aspirant
              </span>
            </div>
      ...
      {activeTab === 'library' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-black text-slate-900">Your Unlocked NEET Material</h2>
          ...
          {filteredLibrary.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 space-y-4">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-800">No notes in your library yet</h3>
              ...
              >
                <span>Explore NEET Notes Marketplace</span>
              </button>
            </div>
          ) : (
                ...
                >
                  <div>
                    <div className="flex items-center justify-between text-xs mb-2">
                      ...
                      <span className="text-[11px] text-slate-400">{note.total_pages} Pages</span>
                    </div>
